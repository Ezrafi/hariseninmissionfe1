...
const Home = () => {
  // --- STEP 1: Inisialisasi State ---
  const titles = ["Big 4 Auditor Financial Analyst", "Mastering React Modern 2024", "Digital Marketing Strategy", "UI/UX Design Fundamental", "Manajemen Bisnis Retail"];
  const instructors = ["Jenna Ortega", "Anya Taylor", "Thomas Shelby", "Margo Robbie"];

  const [courses, setCourses] = useState(() => {
    return Array.from({ length: 9 }, (_, i) => ({
      id: i + 1, // ID Unik untuk setiap data
  ...
  const handleAddCourse = () => {
    ...
    setCourses([newCourse, ...courses]); // Tambah ke urutan paling atas
  };
    ...
    <div className="max-w-[1440px] mx-auto pb-[64px] px-4 lg:px-[120px] flex flex-col gap-[64px] mt-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course) => (
            <CourseCard 
              key={course.id} // PENTING: Gunakan ID unik
              {...course} 
              onDelete={() => handleDeleteCourse(course.id)} // Kirim fungsi ke Card
            />
          ))}
        </div>
      </main>
